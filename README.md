...
<title>ສະຫຼຸບລາຍຮັບ - ລາຍຈ່າຍ</title>
...
<div class="title"><div class="dot credit"></div><div><h3 id="g-credit">CREDIT(IN)+CASH(ເປີດ)	</h3><p>ຍອດເປີດ-ເຄດິດເຂົ້າ</p></div></div>
<table>
<thead><tr><th>ລາຍການ</th><th>ຈໍານວນ</th></tr></thead>
<tbody>
<tr><td class="label">CASH (ເປີດ)</td><td class="value" id="cash_open" contenteditable="true">0</td></tr>
...
<div class="title"><div class="dot cash"></div><div><h3 id="g-cash">ເງິນສົດ / ບັນຊີ</h3><p>ຍອດປິດ-ເຄດິດອອກ</p></div></div>
<table>
<thead><tr><th>ລາຍການ</th><th>ຈໍານວນ</th></tr></thead>
<tbody>
<tr><td class="label">CASH (ປິດ)</td><td class="value" id="cash_close" contenteditable="true">0</td></tr>
...
<div class="title"><div class="dot other"></div><div><h3 id="g-other">ອື່ນໆ (Expenses)</h3><p>ເງີນ-ລາຍຈ່າຍຕ່າງໆ  </p></div></div>
<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-bottom:10px">
<div><div style="font-size:12px;color:var(--muted)"><img src="logo1.jpg" alt="" width="30px">cash(ເງິນສົດ)</div><div id="cash" contenteditable="true" class="value">0</div></div>
<div><div style="font-size:12px;color:var(--muted)"><img src="bin.jpeg" alt="" width="15px">TICKET(ບິນ)</div><div id="ticket" contenteditable="true" class="value">0</div></div>
<div><div style="font-size:12px;color:var(--muted)">promo(ໂປໂມຊັນ)</div><div id="promo" contenteditable="true" class="value">0</div></div>
<div><div style="font-size:12px;color:var(--muted)">expense(ລາຍຈ່າຍ)</div><div id="expense" contenteditable="true" class="value">0</div></div>
<div><div style="font-size:12px;color:var(--muted)">expense(OL)(ລາຍຈ່າຍonline)</div><div id="expenseOL" contenteditable="true" class="value">0</div></div>
<div><div style="font-size:12px;color:var(--muted)"><img src="batbc.png" alt="" width="40px">ບາດ</div><div id="cash2" contenteditable="true" class="value">0</div></div>
<div><div style="font-size:12px;color:var(--muted)"><img src="batldb.png" alt="" width="40px">ບາດ</div><div id="cash3" contenteditable="true" class="value">0</div></div>
<div><div style="font-size:12px;color:var(--muted)">cash4ກີບ</div><div id="cash4" class="value" style="background:#eee;">0</div></div>
...
<div><div style="font-size:12px;color:var(--muted)">cash5ກີບ</div><div id="cash5" class="value" style="background:#eee;">0</div></div>
...
<div><div style="font-size:12px;color:var(--muted)">cash6ກີບ</div><div id="cash6" class="value" style="background:#eee;">0</div></div>
...
<!-- buttons -->
<div class="btns">
<button class="btn ghost" id="pasteData">📋 ວາງຂໍ້ມູນ</button>
<button class="btn ghost" id="copy">📑 Copy CSV</button>
<button class="btn blue" id="downloadImg">🖼️ Download PNG</button>
<button class="btn" id="downloadXlsx" style="background:#0b6b4a;color:#fff">📥 Download XLSX</button>
<button class="btn warn" id="reset">♻️ Reset</button>
<button class="btn green" id="sendWhatsApp">💬 Send WhatsApp</button>
</div>
</div>
...
<div id="confirmModalOverlay" class="modal-overlay" aria-hidden="true">
  <div class="modal" role="dialog" aria-modal="true" aria-labelledby="confirmModalTitle">
    <h3 id="confirmModalTitle">ຢືນຢັນການ Reset?</h3>
    <p>ທ່ານແນ່ໃຈບໍວ່າຈະລ້າງຂໍ້ມູນທັງໝົດ? ການກົດຢືນຢັນຈະຄືນຄ່າເປັນ 0.</p>
    <div class="modal-actions">
      <button id="confirmYes" class="btn warn">ຢືນຢັນ</button>
      <button id="confirmNo" class="btn ghost">ຍົກເລີກ</button>
    </div>
  </div>
</div>
...
<div id="whatsappModalOverlay" class="modal-overlay" aria-hidden="true">
  <div class="modal" role="dialog" aria-modal="true" aria-labelledby="whatsappModalTitle">
    <h3 id="whatsappModalTitle">Preview ຂໍ້ຄວາມ (WhatsApp)</h3>
    <p>ກວດເບິ່ງຂໍ້ຄວາມກ່ອນສົ່ງ — ຕັ້ງເບີ WhatsApp ຈ່າຍບໍ່ຕ້ອງພາຍໃນ (+856...)</p>
    ...
    <div class="modal-actions">
      <button id="confirmSend" class="btn green">✅ ສົ່ງ</button>
      <button id="cancelSend" class="btn ghost">❌ ຍົກເລີກ</button>
    </div>
  </div>
</div>
...
const monthNames = ["ມ.ກ","ກ.ພ","ມ.ນ","ມ.ສ","ພ.ພ","ມິ","ກ.ລ","ສ.ຫ","ກ.ຍ","ຕ.ລ","ພ.ຈ","ທ.ນ"];
function upd(){
  const d=new Date();
...
function calculateAll(){
  ...
  const grand = (parseCell('credit_in')+ parseCell('credit_in2')+ parseCell('credit_in3')+ parseCell('credit_in4')+parseCell('credit_in5')
    - parseCell('credit_out')- parseCell('credit_out2')- parseCell('credit_out3')- parseCell('credit_out4')
    + parseCell('cash_open') - parseCell('cash_close') + parseCell('bank_close'));
...
document.getElementById('pasteData').addEventListener('click', async ()=>{
  try{
    ...
    calculateAll();
  }catch(e){alert('ບໍ່ສາມາດວາງຂໍ້ມູນໄດ້');}
});
...
function buildMessage(){
  ...
  let lines = [];
  lines.push(`📊 ສະຫຼຸບລາຍຮັບ - ລາຍຈ່າຍ (${dateStr})`);
  lines.push('');
  editableIds.forEach(id=>{
    const el = document.getElementById(id);
  ...
  lines.push('');
  lines.push(`TotalExpenses: ${document.getElementById('totalExpenses').textContent} 	฿`);
  lines.push(`Grand Total: ${document.getElementById('grand_total').textContent} 	฿`);
  lines.push(`ສົ່ງອອກຈາກລະບົບ: ${document.getElementById('grand_total').textContent} `);
  return lines.join('\n');
}
...
btnConfirmSend.addEventListener('click', ()=>{
  ...
  if(!phoneClean){
    alert('ໃສ່ເບີ WhatsApp ຼຍ່ອງຖືກ (ເຊັ່ນ 85620xxxxxxx)');
    return;
  }
...
// --- บันทึกค่าลง localStorage ทุกครั้งที่แก้ไข ---
editableIds.forEach(id=>{
  const el=document.getElementById(id);
  if(!el) return;
...
// --- Reset ก็ล้าง localStorage ด้วย ---
btnYes.addEventListener('click', ()=>{
  editableIds.forEach(id=>{
    const el = document.getElementById(id);
    if(el) el.textContent = '0';
